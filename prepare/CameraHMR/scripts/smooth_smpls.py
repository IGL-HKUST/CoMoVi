...
def import_smpls_group(smpls_group_path):
    ...
    for frame in sorted(glob(os.path.join(smpls_group_path, '*.npy')), key=lambda x: int(x.split('/')[-1].split('.')[0])):
        frame_result = np.load(frame, allow_pickle=True).item()
        results["smpl"].append(frame_result["smpls"])
        results["camera"].append(frame_result["cam_t"][0])
        results["render_res"].append(frame_result["render_res"])
        results["scaled_focal_length"].append(frame_result["scaled_focal_length"])
...
def export_smpls_group(smpls_save_path):
    ...
    for frame in range(scene.frame_start, scene.frame_end):
        ...
        cam_result[1] *= -1  # -1 should be multiplied to trans[1] ?
