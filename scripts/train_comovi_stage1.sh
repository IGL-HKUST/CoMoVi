...
# stage-1: pretrain motion branch (adapt wan to smpl video domain)
...
CUDA_VISIBLE_DEVICES=$GPU_IDS accelerate launch --zero_stage 3 --zero3_save_16bit_model true --zero3_init_flag true --use_deepspeed --deepspeed_config_file config/zero_stage3_config_cpu_offload.json --deepspeed_multinode_launcher standard --num_processes $GPU_NUM --num_machines $MACHINE_NUM --machine_rank $LOCAL_RANK --main_process_ip $MAIN_MACHINE_IP train.py \
  ...
  --interaction="none" \
  --interleave=1 \
  --require_grad_modules "motion" \
  --trainable_modules "motion" \
  --rgb_loss_weight=0 \
  --motion_loss_weight=1 \
  --tracker_project_name="example_experiment_stage1" \
