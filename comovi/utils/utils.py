...
def save_videos_grid(
    ...
    videos = [rearrange(video, "b c t h w -> t b c h w") for video in videos]

    outputs = []
    for t in range(videos[0].shape[0]):
        output = []
        for video in videos:
            x = video[t]
...
def save_smpl_npz(smpl_pose, smpl_shape, save_path):
    smpl_pose = rotation_6d_to_matrix(smpl_pose.view(80, 24, 6)).float().cpu().numpy()
    np.savez(save_path, global_orient=smpl_pose[:, :1], betas=smpl_shape, body_pose=smpl_pose[:, 1:])

def save_smpl_mesh(smpl_pose, smpl_shape, save_path, device="cuda", dtype=torch.bfloat16):
    body_model = smplx.SMPLLayer(model_path="./body_model/SMPL/SMPL_NEUTRAL.pkl", num_betas=10).to(device, dtype)
    smpl_info = np.load("./body_model/smpl_mesh_info.npy", allow_pickle=True).item()
    smpl_faces = smpl_info["faces"].astype(np.int32)

    smpl_pose = rotation_6d_to_matrix(smpl_pose.view(80, 24, 6)).to(device, dtype)
    smpl_shape = smpl_shape[1:].to(device, dtype)
    ...
    vertices = smpl_pred_output.vertices.float().cpu().numpy()
    faces = smpl_faces

    os.makedirs(save_path, exist_ok=True)
    for i in range(vertices.shape[0]):
        smpl_mesh = trimesh.Trimesh(vertices=vertices[i], faces=faces)
        smpl_mesh.export(os.path.join(save_path, f"{i}.obj"))
...
def get_image_to_video_latent(validation_image_start, validation_image_end, video_length, sample_size):
    if validation_image_start is not None and validation_image_end is not None:
        ...
        if type(image_start) is list:
            ...
            input_video = torch.tile(
                torch.from_numpy(np.array(image_start)).permute(2, 0, 1).unsqueeze(1).unsqueeze(0), # (h, w, 3) -> (3, h, w) -> (1, 3, 1, h, w) -> (1, 3, f, h, w)
                [1, 1, video_length, 1, 1]
            ) / 255
            input_video_mask = torch.zeros_like(input_video[:, :1])                                 # (1, 1, f, h, w)
            input_video_mask[:, :, 1:, ] = 255
    else:
        image_start = None
        image_end = None
...
def get_smpl_init_pose(npz_path):
    ...
    smpl_poses = torch.from_numpy(np.concatenate([smpl_data["global_orient"], smpl_data["body_pose"]], axis=1))
    smpl_poses = matrix_to_rotation_6d(smpl_poses)
    smpl_shapes = torch.from_numpy(smpl_data["betas"])

    return smpl_poses, smpl_shapes
