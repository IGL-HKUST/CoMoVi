...
from comovi.data.bucket_sampler import ComoviSampler, RandomSampler
from comovi.data.dataset_image_video import ComoviDataset, ImageVideoSampler, get_random_mask
...
from comovi.pipeline import WanPipeline, WanI2VPipeline
from comovi.utils.discrete_sampler import DiscreteSampling
from comovi.utils.utils import get_image_to_video_latent, save_videos_grid
...
def parse_args():
    ...
    parser.add_argument(
        "--uniform_sampling",
        action="store_true",
        help="Whether or not to use uniform_sampling."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--enable_bucket",
        action="store_true",
        help="Whether enable bucket sample in datasets."
    )
    parser.add_argument(
        "--random_ratio_crop",
        action="store_true",
    ...
    parser.add_argument(
        "--random_hw_adapt",
        action="store_true",
        help="Whether enable random adapt height and width in datasets."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--motion_sub_loss",
        action="store_true",
        help="Whether enable motion sub loss."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--allow_missisng_modules',
        nargs='+', 
        default=[],
        help='Enter a list of modules can be missed by given checkpoint'
    )
    parser.add_argument(
        '--rgb_loss_weight', 
        type=float,
    ...
    parser.add_argument(
        "--use_fsdp",
        action="store_true",
        help="Whether or not to use fsdp."
    )
    parser.add_argument(
        "--low_vram",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default="dual",
        choices=["dual", "single_m2v", "single_v2m", "none"]
    )
    parser.add_argument(
        "--interleave",
        type=int,
...
def main():
    ...
    if accelerator.is_main_process:
        writer = SummaryWriter(log_dir=os.path.join(logging_dir, args.tracker_project_name))
    ...
    if accelerator.is_main_process:
        if args.output_dir is not None:
            os.makedirs(os.path.join(args.output_dir, args.tracker_project_name), exist_ok=True)
    ...
    transformer3d = ComoviTransformer3DModel.from_pretrained(
    ...
    # Get smpl body model
    body_model = smplx.SMPLLayer(model_path="./body_model/SMPL/SMPL_NEUTRAL.pkl", num_betas=10).to(accelerator.device).to(weight_dtype)
    ...
    if args.transformer_path:
        print(f"starting from customized checkpoint: {args.transformer_path}")
        ...
        assert len(u) == 0, f"no unexpected module is allowed for given customized checkpoint {args.transformer_path}"
        if len(m) > 0:
            for _m in m:
                allowed = False
                for am in args.allow_missisng_modules:
                    if am in _m:
                        allowed = True
                        break
                assert allowed, f"module {_m} is missed by the given checkpoint, which is not allowed"
        print(f"customized checkpoint: {args.transformer_path} loded successfully")
    ...
    for name, param in transformer3d.named_parameters():
        for trainable_module_name in args.require_grad_modules:
    ...
    trainable_params_optim = [
        {'params': [], 'lr': args.learning_rate, 'name': "_".join(args.trainable_modules)},
        {'params': [], 'lr': 0, 'name': "null" if len(args.trainable_modules_low_learning_rate) == 0 else "_".join(args.trainable_modules_low_learning_rate)}
    ]
    in_already = []
    for name, param in transformer3d.named_parameters():
        high_lr_flag = False
    ...
    sample_n_frames_bucket_interval = vae.config.temporal_compression_ratio # 4
    spatial_compression_ratio = vae.config.spatial_compression_ratio        # 16
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            """
            organize all the training data in fixed resolution, fps, and length
            """
            ...
            new_examples["motion_values"]               = []
            new_examples["firstframe_values"]           = []
            new_examples["motion_firstframe_values"]    = []
            new_examples["smpl_poses"]                  = []
            new_examples["smpl_shapes"]                 = []
            new_examples["text"]                        = []
            # Used in Inpaint mode 
            if args.train_mode != "normal":
                new_examples["mask_pixel_values"]       = []
                new_examples["mask_motion_values"]      = []
                new_examples["mask"]                    = []
            ...
            for example in examples:
                ...
                motion_values = torch.from_numpy(example["motion_values"]).permute(0, 3, 1, 2).contiguous()
                motion_values = motion_values / 255.

                firstframe_values = torch.from_numpy(example["first_frame"]).permute(0, 3, 1, 2).contiguous()
                firstframe_values = firstframe_values / 255.

                motion_firstframe_values = torch.from_numpy(example["motion_first_frame"]).permute(0, 3, 1, 2).contiguous()
                motion_firstframe_values = motion_firstframe_values / 255.

                smpl_poses = torch.from_numpy(np.concatenate([example["smpl_values"]["global_orient"], example["smpl_values"]["body_pose"]], axis=1))
                smpl_poses = matrix_to_rotation_6d(smpl_poses)
                smpl_shapes = torch.from_numpy(example["smpl_values"]["betas"])

                transform = transforms.Compose([
                    transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5], inplace=True),
                ])
                new_examples["pixel_values"].append(transform(pixel_values))
                new_examples["motion_values"].append(transform(motion_values))
                new_examples["firstframe_values"].append(transform(firstframe_values))
                new_examples["motion_firstframe_values"].append(transform(motion_firstframe_values))
                new_examples["smpl_poses"].append(smpl_poses)
                new_examples["smpl_shapes"].append(smpl_shapes)
                ...
                if args.train_mode != "normal":
                    ...
                    mask_pixel_values = new_examples["firstframe_values"][-1].repeat(mask.shape[0], 1, 1, 1) * (1 - mask)
                    mask_motion_values = new_examples["motion_firstframe_values"][-1].repeat(mask.shape[0], 1, 1, 1) * (1 - mask)
                    ...
                    new_examples["mask_motion_values"].append(mask_motion_values)
            ...
            new_examples["motion_values"] = torch.stack([example[:batch_video_length] for example in new_examples["motion_values"]])
            new_examples["smpl_poses"] = torch.stack([example[:batch_video_length] for example in new_examples["smpl_poses"]])
            new_examples["smpl_shapes"] = torch.stack([example[:batch_video_length] for example in new_examples["smpl_shapes"]])
            if args.train_mode != "normal":
                new_examples["mask_pixel_values"] = torch.stack([example[:batch_video_length] for example in new_examples["mask_pixel_values"]])
                new_examples["mask_motion_values"] = torch.stack([example[:batch_video_length] for example in new_examples["mask_motion_values"]])
    ...
    if args.resume_from_checkpoint:
        ...
        if args.resume_from_checkpoint != "latest":
            ...
            dirs = os.listdir(os.path.join(args.output_dir, args.tracker_project_name))
        ...
        if path is None:
            ...
            pkl_path = os.path.join(os.path.join(args.output_dir, args.tracker_project_name, path), "sampler_pos_start.pkl")
            if os.path.exists(pkl_path):
                with open(pkl_path, 'rb') as file:
            ...
            accelerator.print(f"Resuming from checkpoint {os.path.join(args.output_dir, args.tracker_project_name, path)}")
            accelerator.load_state(os.path.join(args.output_dir, args.tracker_project_name, path))
    else:
        initial_global_step = 0
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        train_loss_log = 0.0; rgb_loss_log = 0.0; motion_loss_log = 0.0
        pose_loss_log = 0.0; vertices_loss_log = 0.0; joint3d_loss_log = 0.0
        batch_sampler.sampler.generator = torch.Generator().manual_seed(args.seed + epoch)
        for step, batch in enumerate(train_dataloader):
            # Data batch sanity check
            if epoch == first_epoch and step == 0 and accelerator.is_main_process:
                pixel_values, motion_values, texts = batch["pixel_values"].cpu(), batch["motion_values"].cpu(), batch["text"]
                pixel_values = rearrange(pixel_values, "b f c h w -> b c f h w")
                motion_values = rearrange(motion_values, "b f c h w -> b c f h w")
                os.makedirs(os.path.join(args.output_dir, "sanity_check"), exist_ok=True)
                for idx, (pixel_value, motion_value, text) in enumerate(zip(pixel_values, motion_values, texts)):
                    pixel_value = pixel_value[None, ...]
                    motion_value = motion_value[None, ...]
                    gif_name = '-'.join(text.replace('/', '').split()[:10]) if not text == '' else f'{global_step}-{idx}'
                    save_videos_grid([pixel_value, motion_value], f"{args.output_dir}/sanity_check/{gif_name[:10]}.gif", rescale=True)
                if args.train_mode != "normal":
                    mask_pixel_values, mask_motion_values, texts = batch['mask_pixel_values'].cpu(), batch['mask_motion_values'].cpu(), batch['text']
                    mask_pixel_values = rearrange(mask_pixel_values, "b f c h w -> b c f h w")
                    mask_motion_values = rearrange(mask_motion_values, "b f c h w -> b c f h w")
                    for idx, (pixel_value, motion_value, text) in enumerate(zip(mask_pixel_values, mask_motion_values, texts)):
                        pixel_value = pixel_value[None, ...]
                        motion_value = motion_value[None, ...]
                        save_videos_grid([pixel_value, motion_value], f"{args.output_dir}/sanity_check/mask_{gif_name[:10] if not text == '' else f'{global_step}-{idx}'}.gif", rescale=True)
            
            with accelerator.accumulate(transformer3d):
                # Convert images to latent space
                pixel_values = batch["pixel_values"].to(weight_dtype)
                motion_values = batch["motion_values"].to(weight_dtype)

                # sample which layer to predict 3D smpl
                layer_smpl_predict_idx = [len(transformer3d.motion_blocks)-1] + rng.choice([layer_i for layer_i in range(len(transformer3d.motion_blocks)-1)], size=args.predict_smpl_num-1, replace=False).tolist()
                smplpose_values = batch["smpl_poses"].to(weight_dtype)
                smplshape_values = batch["smpl_shapes"].to(weight_dtype)
                ...
                if args.train_mode != "normal":
                    ...
                    mask_motion_values = batch["mask_motion_values"].to(weight_dtype)
                    ...
                    if args.auto_tile_batch_size and args.training_with_video_token_length and zero_stage != 3:
                        if args.video_sample_n_frames * args.token_sample_size * args.token_sample_size // 16 >= pixel_values.size()[1] * pixel_values.size()[3] * pixel_values.size()[4]:
                            ...
                            mask_motion_values = torch.tile(mask_motion_values, (4, 1, 1, 1, 1))
                            mask = torch.tile(mask, (4, 1, 1, 1, 1))
                        elif args.video_sample_n_frames * args.token_sample_size * args.token_sample_size // 4 >= pixel_values.size()[1] * pixel_values.size()[3] * pixel_values.size()[4]:
                            mask_pixel_values = torch.tile(mask_pixel_values, (2, 1, 1, 1, 1))
                            mask_motion_values = torch.tile(mask_motion_values, (2, 1, 1, 1, 1))
                ...
                if args.keep_all_node_same_token_length:
                    ...
                    if args.train_mode != "normal":
                        ...
                        mask_motion_values = mask_motion_values[:, :actual_video_length, :, :]
                ...
                with torch.no_grad():
                    ...
                    if vae_stream_1 is not None:
                        ...
                        with torch.cuda.stream(vae_stream_1):
                            ...
                            motion_latents = _batch_encode_vae(motion_values)
                    else:
                        latents = _batch_encode_vae(pixel_values)           # (b, 48, 21, 30, 52)
                        motion_latents = _batch_encode_vae(motion_values)   # (b, 48, 21, 30, 52)

                    if args.train_mode != "normal":
                        mask = rearrange(mask, "b f c h w -> b c f h w")                                    # (1, 1, 81, 480, 832)
                        mask = torch.concat(                                                                # (1, 1, 84, 480, 832)
                            [
                                torch.repeat_interleave(mask[:, :, 0:1], repeats=4, dim=2), 
                                mask[:, :, 1:]
                            ], dim=2
                        )
                        mask = mask.view(mask.shape[0], mask.shape[2] // 4, 4, mask.shape[3], mask.shape[4])# (1, 21, 4, 480, 832)
                        mask = mask.transpose(1, 2)                                                         # (1, 4, 21, 480, 832)
                        ...
                        if args.train_mode != "ti2v":
                            ...
                            # (1, 1, 21, 30, 52)
                            # mask[:, :, :1] is 0; mask[:, :, 1:] is 1
                        
                        # Encode inpaint latents.
                        rgb_mask_latents = _batch_encode_vae(mask_pixel_values)
                        motion_mask_latents = _batch_encode_vae(mask_motion_values)
                        if vae_stream_2 is not None:
                            torch.cuda.current_stream().wait_stream(vae_stream_2) 

                        if args.train_mode != "ti2v":
                            rgb_inpaint_latents = torch.concat([mask, rgb_mask_latents], dim=1)
                            rgb_inpaint_latents = t2v_flag[:, None, None, None, None] * rgb_inpaint_latents

                            motion_inpaint_latents = torch.concat([mask, motion_mask_latents], dim=1)
                            motion_inpaint_latents = t2v_flag[:, None, None, None, None] * motion_inpaint_latents
                        else:
                            rgb_inpaint_latents = rgb_mask_latents
                            motion_inpaint_latents = motion_mask_latents
                ...
                noise_shape = (bsz, channel*2, num_frames, height, width)
                noise = torch.randn(noise_shape, device=latents.device, generator=torch_rng, dtype=weight_dtype)    # (b, 96, 21, 30, 52)
                ...
                noisy_latents = (1.0 - sigmas) * latents + sigmas * noise[:, :channel, :, :, :]                    
                noisy_motion_latents = (1.0 - sigmas) * motion_latents + sigmas * noise[:, channel:, :, :, :]

                # Add noise
                target_rgb = noise[:, :channel, :, :, :] - latents
                target_motion = noise[:, channel:, :, :, :] - motion_latents
                target = torch.cat([target_rgb, target_motion], dim=1)
                
                target_shape = (channel*2, num_frames, width, height)
                seq_len = math.ceil(
                ...
                if args.train_mode == "ti2v":
                    t2v_in_ti2v = 1 # first image condition

                    mask_bs = mask.size()[0]
                    if t2v_in_ti2v:
                        noisy_latents = (1 - mask) * rgb_inpaint_latents + mask * noisy_latents
                        noisy_motion_latents = (1 - mask) * motion_inpaint_latents + mask * noisy_motion_latents
                ...
                with torch.cuda.amp.autocast(dtype=weight_dtype), torch.cuda.device(device=accelerator.device):
                    if args.motion_only:
                        noise_pred = transformer3d.forward_motion_only(
                            x=[noisy_latents, noisy_motion_latents],
                        ...
                        noise_pred, smpl_pred = transformer3d(
                            x=[noisy_latents, noisy_motion_latents],
                            context=prompt_embeds,
                            t=timesteps,
                            seq_len=seq_len,
                            y=None,
                            init_smpl_pose=smplpose_values[:, :1].repeat(1, 80, 1, 1).flatten(2) if args.predict_smpl else None,
                            layer_predict_smpl_idx=layer_smpl_predict_idx if args.predict_smpl else None
                        )
                        # noise_pred: (1, 96, 21, 44, 80)
                        # smpl_pred: (1, args.predict_smpl_num, 80, 24*6)
                ...
                rgb_loss = custom_mse_loss(noise_pred[:, :channel, 1:, :, :].float(), target[:, :channel, 1:, :, :].float(), weighting.float())
                motion_loss = custom_mse_loss(noise_pred[:, channel:, 1:, :, :].float(), target[:, channel:, 1:, :, :].float(), weighting.float())
                loss = args.rgb_loss_weight * rgb_loss.mean() + args.motion_loss_weight * motion_loss.mean()
                
                if args.predict_smpl:
                    pose_loss = custom_mse_loss(smpl_pred.float(), smplpose_values.unsqueeze(1)[:, :, 1:].flatten(3).float(), weighting.float())
                    loss += args.pose_loss_weight * pose_loss.mean()
                    
                    smpl_bs, smpl_f, _ = smplshape_values.shape
                    smpl_ln = smpl_pred.shape[1]
                    smplshape_values = smplshape_values[:, 1:].view(smpl_bs*(smpl_f-1), 10)                             # (80, 10)
                    smplpose_values = rotation_6d_to_matrix(smplpose_values[:, 1:].view(smpl_bs*(smpl_f-1), 24, 6))     # (80, 24, 3, 3)
                    smpl_gt_output = body_model(
                        global_orient=smplpose_values[:, :1, :, :],
                        betas=smplshape_values,
                        body_pose=smplpose_values[:, 1:, :, :]
                    )
                    smpl_gt_vertices = smpl_gt_output.vertices.view(smpl_bs, smpl_f-1, 6890, 3).unsqueeze(1)            # (1, 1, 80, 6890, 3)
                    smpl_gt_joints = smpl_gt_output.joints[:, :24, :] - smpl_gt_output.joints[:, :1, :]      
                    smpl_gt_joints = smpl_gt_joints.view(smpl_bs, smpl_f-1, 24, 3).unsqueeze(1)                         # (1, 1, 80, 24, 3)

                    smpl_pred = rotation_6d_to_matrix(smpl_pred.view(smpl_bs*smpl_ln*(smpl_f-1), 24, 6))                # (1*30*80, 24, 3, 3)
                    smpl_pred_output = body_model(
                        global_orient=smpl_pred[:, :1, :, :],
                        betas = smplshape_values.repeat(smpl_ln, 1),
                        body_pose = smpl_pred[:, 1:, :, :]
                    )
                    smpl_pred_vertices = smpl_pred_output.vertices.view(smpl_bs, smpl_ln, smpl_f-1, 6890, 3)            # (1, 30, 80, 6890, 3)
                    smpl_pred_joints = smpl_pred_output.joints[:, :24, :] - smpl_pred_output.joints[:, :1, :]
                    smpl_pred_joints = smpl_pred_joints.view(smpl_bs, smpl_ln, smpl_f-1, 24, 3)
                    
                    vertices_loss = custom_mse_loss(smpl_pred_vertices.float(), smpl_gt_vertices.float(), weighting.float())
                    loss += args.vertices_loss_weight * vertices_loss.mean()
                    
                    joint3d_loss = custom_mse_loss(smpl_pred_joints.float(), smpl_gt_joints.float(), weighting.float())
                    loss += args.joint3d_loss_weight * joint3d_loss.mean()
                ...
                avg_motion_loss = accelerator.gather(motion_loss.repeat(args.train_batch_size)).mean()
                train_loss_log += avg_loss.item() / args.gradient_accumulation_steps
                rgb_loss_log += avg_rgb_loss.item() / args.gradient_accumulation_steps
                motion_loss_log += avg_motion_loss.item() / args.gradient_accumulation_steps
                if args.predict_smpl:
                    avg_pose_loss = accelerator.gather(pose_loss.repeat(args.train_batch_size)).mean()
                    avg_vertices_loss = accelerator.gather(vertices_loss.repeat(args.train_batch_size)).mean()
                    avg_joint3d_loss = accelerator.gather(joint3d_loss.repeat(args.train_batch_size)).mean()
                    pose_loss_log += avg_pose_loss.item() / args.gradient_accumulation_steps
                    vertices_loss_log += avg_vertices_loss.item() / args.gradient_accumulation_steps
                    joint3d_loss_log += avg_joint3d_loss.item() / args.gradient_accumulation_steps
            ...
            if accelerator.sync_gradients:
                ...
                global_step += 1
                log_dict = {
                    "loss/total_loss": train_loss_log,
                    "loss/rgb_loss": rgb_loss_log,
                    "loss/motion_loss": motion_loss_log,
                    "loss/pose_loss": pose_loss_log if args.predict_smpl else 0,
                    "loss/vertices_loss": vertices_loss_log if args.predict_smpl else 0,
                    "loss/joint3d_loss": joint3d_loss_log if args.predict_smpl else 0,
                    "metric/gradient_norm": norm_sum,
                    "metric/rgb_loss_weight": args.rgb_loss_weight,
                    "metric/motion_loss_weight": args.motion_loss_weight,
                    "metric/pose_loss_weight": args.pose_loss_weight,
                    "metric/vertices_loss_weight": args.vertices_loss_weight,
                    "metric/joint3d_loss_weight": args.joint3d_loss_weight
                }
                for pg in optimizer.param_groups:
                    log_dict.update({f"learning_rate/{pg['name']}": pg["lr"]})
                accelerator.log(log_dict, step=global_step)
                train_loss_log = 0.0; rgb_loss_log = 0; motion_loss_log = 0
                pose_loss_log = 0.0; vertices_loss_log = 0.0; joint3d_loss_log = 0

                if global_step % args.checkpointing_steps == 0:
                    if args.use_deepspeed or args.use_fsdp or accelerator.is_main_process:
                        # _before_ saving state, check if this save would set us over the `checkpoints_total_limit`
                        if args.checkpoints_total_limit is not None and accelerator.is_main_process:
                            checkpoints = os.listdir(os.path.join(args.output_dir, args.tracker_project_name))
                            checkpoints = [d for d in checkpoints if (d.startswith("checkpoint") and (not d.endswith("outputs")))]
                            ...
                            if len(checkpoints) >= args.checkpoints_total_limit:
                                ...
                                for removing_checkpoint in removing_checkpoints:
                                    removing_checkpoint = os.path.join(args.output_dir, args.tracker_project_name, removing_checkpoint)
                                    shutil.rmtree(removing_checkpoint)

                        gc.collect()
                        torch.cuda.empty_cache()
                        torch.cuda.ipc_collect()
                        save_path = os.path.join(args.output_dir, args.tracker_project_name, f"checkpoint-{global_step}")
            ...
            logs = {
                ...
                "pose_loss": pose_loss.detach().item() if args.predict_smpl else 0,
                "vertices_loss": vertices_loss.detach().item() if args.predict_smpl else 0,
                "joint3d_loss": joint3d_loss.detach().item() if args.predict_smpl else 0,
